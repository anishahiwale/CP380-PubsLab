    ...
    class Program
    {
        static void Main(string[] args)
        {
            using (var dbcontext = new Models.PubsDbContext())
            {
                ...
                var eachEmpJobs = dbcontext.Employees.Select(e => new { EmpId = e.emp_id, FName = e.fname, LName = e.lname ,Job_Desc = e.Jobs.job_desc }).ToList();
				
				Console.WriteLine("\n====== Employee ID, Employee Name, Job Description ======");
                foreach (var i in eachEmpJobs)
                {
